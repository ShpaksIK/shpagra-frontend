    ...
    return (
        <div className={style.notificationPreview}>
            ...
            <div className={style.notificationPreview__footer}>
                {notification.type === 'article' && <A to={`/article/${notification.relatedId}`} text='Перейти' />}
                {notification.type === 'post' && <A to={`/post/${notification.relatedId}`} text='Перейти' />}
                {notification.type === 'profile' && <A to={`/profile/${notification.relatedId}`} text='Перейти' />}
